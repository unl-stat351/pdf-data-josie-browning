...
classics_lines <- classics_pages %>%
    filter((x >= 0 & x <= 385) | (x >= 720 & x <= 775),
           y >= 153)
    group_by(id, page, y) %>% #group by line, or observations with the same y
    summarise(line = paste(text, collapse = " "), .groups = "drop")

titles <- c("Professor", "Associate Professor", "Graduate Teaching Asst", "Chairperson", "Student Worker", "Assistant Professor", "Assistant Professor of Practice", "Associate Professor of Practice", "Sr. Lecturer", "Staff Secy III")
...
classics_dpt <- classics_lines %>%
    mutate(
        ...
        extracted_title = str_extract(line, title_pattern),
        title = extracted_title,
        name = str_remove(name, title_pattern) |> str_squish())


    fill(name, title, .direction = "downup") %>%
