...
> Some of the things I want to address first is spacing
>
> Knowing that the indentations and odd spacing might cause issues in how columns are read, I think I will need to do something to address the spacing in the metadata. Anything that can be done so that the text data can be more easily read into a tabular structure should probably be done.
...
> It looks like there is some differences between the files, likely due to them being created over a long period of time and therefore requiring different version/types of programs. The most recent pdf uses Abode Acrobat instead of Acrobat Distiller or PScript5, which could cause issues. Another notable difference is that only two files are linearized (2015 and 2020). After some reseach I understand that linearized pdfs have internal structures that are designed to have pages load faster for viewing, versus all the pages loading first before they can be viewed. As of right now, I believe that this will not cause any issues in how r handles the pdfs. 
...
read_salary_data <- function(file, start, end) {
  ...
  # Combine into one character vector of lines
  lines <- str_split(target_pages, "\n") %>%
    ...
    max_cols <- max(map_int(split_lines, length))
  
    df <- map_dfr(split_lines, ~ as_tibble_row(set_names(lengthen(.x, max_cols), paste0("V", 1:max_cols))))
  
    names(df) <- make.names(unlist(df[which.max(rowSums(!is.na(df[1:10,]))),]), unique = TRUE)
  
    df[-(1:which.max(rowSums(!is.na(df[1:10,])))), ] |>
        mutate(file = basename(file), page_start = start, page_end = end) |>
        relocate(file, page_start, page_end)
}


file <- "pdf_files/unl-department-budget-2025-2026.pdf"

salary_data_2025 <- read_salary_data(file, start = 12, end = 20)

head(salary_data_2025)
```
