...
classics_dpt <- classics_lines %>%
  filter(str_detect(line, "\\d{1,3}(?:,\\d{3})+")) %>% #numeric that have commas, like 210,010
  mutate(
      salary_text = str_extract(line, "\\d{1,3}(?:,\\d{3})+"),
      salary = parse_number(salary_text),
      name = str_extract(line, "[A-Z][A-Za-z'\\-]+,\\s+[A-Z][A-Za-z\\.\\s\\-]{1,60}"),
      title = str_extract(line,
       "(?i)\\b(Professor|Associate Professor|Graduate Teaching Asst|Chairperson|Student Worker|Assistant Professor| Assistant Professor of Practice|Associate Professor of Practice|Sr. Lecturer|Staff Secy III )\\b")
  ) #found these manually bc I couldn't figure it out
#issue write now is dealing with subtotals/multiple rows for each faculty

classics_dpt <- classics_dpt %>%
  group_by(name) %>%
  mutate(title = if_else(all(is.na(title)), 
                         NA_character_, first(na.omit(title)))) %>%   #fill titles for people w multiple rows
  ungroup()

people_salaries <- classics_dpt %>%
